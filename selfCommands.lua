...
-- 
-- General functions for manipulating state values via self-commands. Only handles certain specific
-- states that we've defined, though it allows the user to hook into the cycle command.
-----------------------------------------------------------------------------------------------------------
...
function self_command(commandArgs)
	...
	local eventArgs = {handled = false}						--init a new eventArgs

	if job_self_command then								--If jobs are allowed to use their own command handlers
		job_self_command(commandArgs, eventArgs)			--Pass the arguments to the job specific handler
	end

	if not eventArgs.handled then							--If not handled by job specific handler
		local handleCmd = table.remove(commandArgs, 1)		--Use arg1 as the name of the command to execute
		if selfCommandMaps[handleCmd] then					--If there is a handler for that command
			selfCommandMaps[handleCmd](commandArgs)			--Pick the handler to use, and pass it arg2 as the argument for that command
		end
	end
end


function equip_set(cmdParams)
	local eset
	if cmdParams[1] then
		eset = cmdParams[1]:lower()
	end
	
	if eset then	--Do nothing if no parameter was given
		if eset == 'engaged' then
			equip_gear_by_status('Engaged')
			--equip(get_current_melee_set())
		elseif eset == 'idle' then
			equip_gear_by_status('Idle')
		end
	end
end
...
function addToChat(cmdParams)
	...
	for i = 3, table.getn(cmdParams), 1 do
...
function get_mode_list(field)
	...
	elseif field == 'Accuracy' then
		modeList = options.AccuracyModes
		currentValue = state.AccuracyMode
...
function set_mode(field, val)
	if field == 'Offense'				then	state.OffenseMode = val
	elseif field == 'Accuracy'			then	state.AccuracyMode = val
	elseif field == 'Defense'			then	state.DefenseMode = val
	elseif field == 'Ranged'			then	state.RangedMode = val
	elseif field == 'Casting'			then	state.CastingMode = val
	elseif field == 'Weaponskill'		then	state.WeaponskillMode = val
	elseif field == 'Idle'				then	state.IdleMode = val
	elseif field == 'Resting'			then	state.RestingMode = val
	elseif field == 'Physicaldefense'	then	state.Defense.PhysicalMode = val
	elseif field == 'Magicaldefense'	then	state.Defense.MagicalMode = val
	elseif field == 'Target'			then	state.PCTargetMode = val
...
function equip_fishing_gear()
	local fishSet = get_fishing_gear()
	equip(fishSet)
end
